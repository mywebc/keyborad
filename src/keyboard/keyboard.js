	...
	inputOn(inputEle, type) {
		...
		for (let i = 0; i < elKeysEl.length; i++) {
			switch (elKeysEl[i].innerHTML) {
				...
				case '⇦':
					elKeysEl[i].onclick = function () {
						const curIndex = _this.getPosition(inputEl)
						inputEl.setSelectionRange(curIndex, curIndex)
						inputEl.focus()
						inputEl[type] = inputEl[type].slice(0, curIndex - 1) + inputEl[type].slice(curIndex);
						inputEl.setSelectionRange(curIndex - 1, curIndex - 1)
						inputEl.focus()
					}
					continue;
				...
				case 'Enter':
					elKeysEl[i].onclick = function () {
						...
						inputEl.setSelectionRange(curIndex, curIndex)
						inputEl.focus()
						inputEl[type] = inputEl[type].slice(0, curIndex) + '\n' + inputEl[type].slice(curIndex);
						inputEl.setSelectionRange(curIndex + 1, curIndex + 1)
						inputEl.focus()
				...
				case 'Space':
					elKeysEl[i].onclick = function () {
						...
						inputEl.setSelectionRange(curIndex, curIndex)
						inputEl.focus()
						inputEl[type] = inputEl[type].slice(0, curIndex) + ' ' + inputEl[type].slice(curIndex);
						inputEl.setSelectionRange(curIndex + 1, curIndex + 1)
						inputEl.focus()
						// inputEl[type] += ' ';
					}
			...
			elKeysEl[i].onclick = function () {
				...
				inputEl.setSelectionRange(curIndex, curIndex)
				inputEl.focus()
				inputEl[type] = inputEl[type].slice(0, curIndex) + this.innerText + inputEl[type].slice(curIndex);
				inputEl.setSelectionRange(curIndex + 1, curIndex + 1)
				inputEl.focus()
			}
		}
